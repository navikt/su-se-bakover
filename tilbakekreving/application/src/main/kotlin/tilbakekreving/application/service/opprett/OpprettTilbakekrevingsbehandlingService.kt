...
import tilbakekreving.domain.opprettelse.opprettTilbakekrevingsbehandlingUtenKravgrunnlag
...
class OpprettTilbakekrevingsbehandlingService(
    ...
    fun opprettUten(
        command: OpprettTilbakekrevingsbehandlingCommand,
    ): Either<KunneIkkeOppretteTilbakekrevingsbehandling, OpprettetTilbakekrevingsbehandlingUtenKravgrunnlag> {
        val sakId = command.sakId
        tilgangstyring.assertHarTilgangTilSak(sakId).onLeft {
            log.info("Kunne ikke opprette tilbakekreving. Mangler tilgang til sak. command: $command. Feil: $it")
            return KunneIkkeOppretteTilbakekrevingsbehandling.IkkeTilgang(it).left()
        ...
        return when (sak.uteståendeKravgrunnlag) {
            null -> {
                opprettTilbakekrevingsbehandlingUtenKravgrunnlag(
                    command = command,
                    forrigeVersjon = sak.versjon,
                    clock = clock,
                    fnr = sak.fnr,
            ...
            else -> throw IllegalStateException("Skal ikke opprette behandling uten kravgrunnlag hvis det finnes kravgrunnlag")
        }
    }
}
