...
For `su-se-bakover`, compare `.nais/dev-gcp.yaml`, `.nais/dev-gcp-q1.yaml` and
`.nais/prod-gcp.yaml` and preserve their established naming, resources, auth and
access policies. Generic templates and resource sizes below are references, not
defaults. Ask before commands or changes against production.
