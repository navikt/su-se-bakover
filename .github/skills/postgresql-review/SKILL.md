...
SELECT bruker_id, vedtak_id, opprettet_dato
FROM (
    SELECT
        bruker_id,
        vedtak_id,
...
Review the existing datasource configuration and total connection budget across
replicas. Do not replace it with a generic Hikari configuration or change pool size
...
- [ ] Is the existing connection pool appropriate for replicas and database limits?
