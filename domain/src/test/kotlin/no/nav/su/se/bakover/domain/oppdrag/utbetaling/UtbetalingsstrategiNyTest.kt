...
internal class UtbetalingsstrategiNyTest {
            ...
            beregning = BeregningFactory.ny(
                ...
                fradrag = listOf(
                    ...
                    FradragFactory.ny(
                        ...
                        månedsbeløp = 4000.0,
                        periode = Periode.create(fraOgMed = 1.februar(2020), tilOgMed = 31.mars(2020)),
                        utenlandskInntekt = null,
                        tilhører = FradragTilhører.BRUKER,
                    ),
                ),
    ...
    fun `mapper flere beregningsperioder til flere ufregrunnlag (mange til mange)`() {
        ...
        actual.utbetalingslinjer.size shouldBe 4
        actual shouldBe expectedUtbetaling(
            actual = actual,
            oppdragslinjer = nonEmptyListOf(
                expectedUtbetalingslinje(
                    utbetalingslinjeId = actual.utbetalingslinjer[0].id,
                ...
                expectedUtbetalingslinje(
                    ...
                    tilOgMed = 31.mai(2021),
                    beløp = 15989,
                    forrigeUtbetalingslinjeId = actual.utbetalingslinjer[0].id,
                    uføregrad = Uføregrad.parse(50),
                ),
                expectedUtbetalingslinje(
                    utbetalingslinjeId = actual.utbetalingslinjer[2].id,
                    opprettet = actual.utbetalingslinjer[2].opprettet,
                    fraOgMed = 1.juni(2021),
                    tilOgMed = 30.juni(2021),
                    beløp = 15989,
                    forrigeUtbetalingslinjeId = actual.utbetalingslinjer[1].id,
                ...
                expectedUtbetalingslinje(
                    utbetalingslinjeId = actual.utbetalingslinjer[3].id,
                    opprettet = actual.utbetalingslinjer[3].opprettet,
                    fraOgMed = 1.juli(2021),
                    tilOgMed = 31.desember(2021),
                    beløp = 12989,
                    forrigeUtbetalingslinjeId = actual.utbetalingslinjer[2].id,
                    uføregrad = Uføregrad.parse(70),
                ),
            ),
        )
