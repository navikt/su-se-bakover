...
class BosituasjonTest {
    ...
    fun `viser om søker har ektefelle eller ikke`(){
        Grunnlag.Bosituasjon.Fullstendig.EktefellePartnerSamboer.Under67.UførFlyktning(
            ...
            begrunnelse = null).harEktefelle() shouldBe true
        ...
        Grunnlag.Bosituasjon.Fullstendig.Enslig(
            ...
            opprettet = Tidspunkt.now(),
            periode = Periode.create(fraOgMed = 1.januar(2021), tilOgMed = 30.juni(2021)),
            begrunnelse = null).harEktefelle() shouldBe false
        ...
        Grunnlag.Bosituasjon.Ufullstendig.HarIkkeEps(
            id = UUID.randomUUID(),
            opprettet = Tidspunkt.now(),
            periode = Periode.create(fraOgMed = 1.januar(2021), tilOgMed = 30.juni(2021))).harEktefelle() shouldBe false

        Grunnlag.Bosituasjon.Ufullstendig.HarEps(
            id = UUID.randomUUID(),
            opprettet = Tidspunkt.now(),
            fnr = FnrGenerator.random(),
            periode = Periode.create(fraOgMed = 1.januar(2021), tilOgMed = 30.juni(2021))).harEktefelle() shouldBe true
    }

    @Test
    fun `returnerer false hvis 2 grunnlag har samme informasjon om ektefelle`(){
        val gjeldendeBosituasjon = Grunnlag.Bosituasjon.Ufullstendig.HarEps(id =UUID.randomUUID(),
            opprettet = Tidspunkt.now(),
            periode = Periode.create(fraOgMed = 1.januar(2021), tilOgMed = 30.juni(2021)),
            fnr = FnrGenerator.random())
    ...
    fun `returnerer true hvis grunnlag om ektefelle har endret sig`(){
        ...
        Grunnlag.Bosituasjon.Ufullstendig.HarIkkeEps(id =UUID.randomUUID(),
            opprettet = Tidspunkt.now(),
            periode = Periode.create(fraOgMed = 1.januar(2021), tilOgMed = 30.juni(2021))).harEndretEllerFjernetEktefelle(gjeldendeBosituasjon) shouldBe true
    }
}
