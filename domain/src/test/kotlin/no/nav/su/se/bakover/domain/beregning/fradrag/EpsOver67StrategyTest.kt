...
internal class EpsOver67StrategyTest {
    ...
    fun `inkluderer ikke fradrag for EPS som er lavere enn ordinært garantipensjonsnivå for aktuell måned`() {
        ...
        val epsArbeidsinntekt = lagPeriodisertFradrag(FradragskategoriWrapper(Fradragskategori.Arbeidsinntekt), 5000.0, periode, tilhører = EPS)
    ...
    fun `inkluderer fradrag for EPS som overstiger ordinært garantipensjonsnivå for aktuell måned`() {
        val periode = månedsperiodeJanuar2020
        val forventetInntekt = lagPeriodisertFradrag(FradragskategoriWrapper(Fradragskategori.ForventetInntekt), 12000.0, periode, tilhører = BRUKER)
        val epsArbeidsinntekt = lagPeriodisertFradrag(FradragskategoriWrapper(Fradragskategori.Arbeidsinntekt), 20000.0, periode, tilhører = EPS)

        val expectedEpsFradrag =
            lagPeriodisertFradrag(FradragskategoriWrapper(Fradragskategori.BeregnetFradragEPS), 20000.0 - 14674.916666666666667, periode, tilhører = EPS)
    ...
    fun `varierer mellom å inkludere og ikke inkludere EPS sine fradrag`() {
        ...
        val epsArbeidsinntektJan =
            lagFradrag(FradragskategoriWrapper(Fradragskategori.Arbeidsinntekt), 20000.0, månedsperiodeJanuar2020, tilhører = EPS)
    ...
    fun `inneholder bare en fradragstype for EPS uavhengig av hvor mange som sendes sinn`() {
        ...
        FradragStrategy.EpsOver67År.beregn(
            ...
            it shouldHaveSize 4
            it.values.forEach {
                it.filter { it.tilhører == EPS }.all { it.fradragskategoriWrapper.kategori == Fradragskategori.BeregnetFradragEPS }
            }
        }
    }
