...
fun Attesteringshistorikk.toFunksjonellTid(
    ...
    return this.prøvHentSisteAttestering()?.opprettet ?: Tidspunkt.now(clock).also {
        log.error("Kunne ikke avgjøre funksjonellTid (defaulter til Tidspunkt.now(clock)) ved statistikkhendelse med behandlingId $id - kunne ikke hente siste attestering")
    }
}
