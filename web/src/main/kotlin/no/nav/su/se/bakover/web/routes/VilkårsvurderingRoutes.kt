                ...
                right = { id ->
                    ...
                    when (val behandling = repo.hentBehandling(id)) {
                        null -> call.svar(HttpStatusCode.NotFound.message("Fant ikke behandling med id:$id"))
                            ...
                            val vilkårsvurderinger = mapper.readValue<Map<String, VilkårsvurderingData>>(
                                call.receiveTextUTF8(),
                                mapper.typeFactory.constructMapType(
                                    HashMap::class.java,
                                    String::class.java,
                                    VilkårsvurderingData::class.java
                                )
                            )

                            behandling.oppdaterVilkårsvurderinger(vilkårsvurderinger.toVilkårsvurderinger())
                            call.svar(HttpStatusCode.OK.json(mapper.writeValueAsString(behandling.toDto().toJson())))
                        }
                    }
                }
            )
