...
import no.nav.su.se.bakover.domain.regulering.sisteÅrIPerioden
                ...
                call.withBody<HentFradragRequest> { request ->
                    ...
                    if (harTilgang(personService, sakService, sakId, request.fnr)) {
                        aapClient.hentMaksimumUtenUtbetaling(
                            ...
                            ifRight = { fradrag ->
                                call.audit(request.fnr, AuditLogEvent.Action.SEARCH, null)
                                call.svar(Resultat.json(OK, serialize(fradrag.vedtak.filter { it.sisteÅrIPerioden(request.periode) }.map { it.mapToResponse() })))
                            },
                        )
                    } else {
                        call.respond(HttpStatusCode.Forbidden)
