...
internal fun FradragssjekkKjøring.lagOppsummering(): FradragssjekkOppsummering {
    val sakerMedOpprettetOppgave = resultat.saksresultater.filter { it.status == FradragssjekkSakStatus.OPPGAVE_OPPRETTET }
...
internal fun FradragssjekkKjøring.antallSakerMedUspesifisertÅrsak(): Int {
    return resultat.saksresultater
        .filter { it.status == FradragssjekkSakStatus.OPPGAVE_OPPRETTET }
...
private fun FradragssjekkSakResultat.tilOppgaveårsaker(): List<Oppgaveårsak> {
    if (status != FradragssjekkSakStatus.OPPGAVE_OPPRETTET) return emptyList()

    val eksplisitteÅrsaker = tilEksplisitteOppgaveårsaker()

    if (eksplisitteÅrsaker.isNotEmpty()) {
        return eksplisitteÅrsaker
    }

    return tilFallbackOppgaveårsak()
}

private fun FradragssjekkSakResultat.harUspesifisertÅrsak(): Boolean {
    if (status != FradragssjekkSakStatus.OPPGAVE_OPPRETTET || oppgaveAvvik.isEmpty()) return false
...
private fun FradragssjekkSakResultat.tilFallbackOppgaveårsak(): List<Oppgaveårsak> {
    if (oppgaveAvvik.size != 1 || sjekkplan.sjekkpunkter.size != 1) {
        return emptyList()
    }

    // Eldre kjøringer kan mangle eksplisitt metadata på avviket. Når det bare finnes ett sjekkpunkt,
    // kan vi fortsatt utlede fradragstypen sikkert.
    val sjekkpunkt = sjekkplan.sjekkpunkter.single()
    return listOf(
        Oppgaveårsak(
            sakId = sakId,
