...
internal class SøknadsbehandlingServiceIverksettTest {
    ...
    inner class GenerelleFeil {
        ...
        fun `svarer med feil dersom vi ikke kunne simulere`() {
            ...
            response shouldBe KunneIkkeIverksetteSøknadsbehandling.KontrollsimuleringFeilet(
                KontrollsimuleringFeilet.KunneIkkeSimulere(
        ...
        @Test
        fun `svarer med feil dersom personService ikke finner bruker i PDL`() {
            val (sak, innvilgetTilAttestering) = søknadsbehandlingTilAttesteringInnvilget()
            val fritekstServiceMock = mock<FritekstService> {
                on {
                    hentFritekst(any(), any(), anyOrNull())
            ...
            ) shouldBe KunneIkkeIverksetteSøknadsbehandling.FantIkkeAdresseTilBruker("Fant ikke bruker i PDL").left()
        }
        ...
        fun `hopper over adresse-sjekk når skalSendeVedtaksbrev er false`() {
            ...
            verify(personServiceMock, never()).hentPerson(any(), any())
        }
    }
    ...
    inner class Avslag {
        ...
        fun `attesterer og iverksetter avslag hvis alt er ok`() {
            ...
            verify(serviceAndMocks.fritekstService).hentFritekst(any(), any(), anyOrNull())
            serviceAndMocks.verifyNoMoreInteractions()
        }
    ...
    val utbetalingService: UtbetalingService = mock {
        ...
        if (sakOgSøknadsbehandling != null) {
            doAnswer { invocation ->
                val utbetaling = invocation.getArgument(0) as Utbetaling.UtbetalingForSimulering
                simulerUtbetaling(
                    utbetalingerPåSak = sakOgSøknadsbehandling.first.utbetalinger,
                    utbetalingForSimulering = utbetaling,
                    clock = clock,
                )
            }.whenever(it).simulerUtbetaling(any())
