...
internal class RevurderingBeregnOgSimulerTest {
    ...
    fun `tar med gjeldende grunnlag for avkorting for revurderingsperioden ved beregning`() {
        ...
        (actual as SimulertRevurdering.Innvilget).let { simulert ->
            simulert.grunnlagsdata.fradragsgrunnlag.filter { it.fradragskategoriWrapper.kategori == Fradragskategori.AvkortingUtenlandsopphold }
                .sumOf { it.månedsbeløp } shouldBe expectedAvkorting
            simulert.beregning.getFradrag().filter { it.fradragskategoriWrapper.kategori == Fradragskategori.AvkortingUtenlandsopphold }
                .sumOf { it.månedsbeløp } shouldBe expectedAvkorting
        }
    }
}
